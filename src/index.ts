function renderHtml(string: any) {
  const template = document.createElement("template");
  template.innerHTML = string;
  return document.importNode(template.content, true);
}

function renderSvg(string: any) {
  const g = document.createElementNS("http://www.w3.org/2000/svg", "g");
  g.innerHTML = string;
  return g;
}

export const html = Object.assign(hypertext(renderHtml, (fragment: any) => {
  ...
  return span;
}), {fragment: hypertext(renderHtml, (fragment: any) => fragment)});

export const svg = Object.assign(hypertext(renderSvg, (g: any) => {
  if (g.firstChild === null) return null;
  if (g.firstChild === g.lastChild) return g.removeChild(g.firstChild);
  return g;
}), {fragment: hypertext(renderSvg, (g: any) => {
...
function hypertext(render: any, postprocess: any) {
  return function({raw: strings}: any) {
    let state: any = STATE_DATA;
    let string = "";
    let tagNameStart; // either an open tag or an end tag
    let tagName; // only open; beware nesting! used only for rawtext
    let attributeNameStart: any;
    let attributeNameEnd: any;
    let nodeFilter = 0;
...
function isRawText(tagName: any) {
  return tagName === "script" || tagName === "style" || isEscapableRawText(tagName);
}

function isEscapableRawText(tagName: any) {
  return tagName === "textarea" || tagName === "title";
}

function lower(input: any, start: any, end: any) {
  return input.slice(start, end).toLowerCase();
}

function setAttribute(node: any, name: any, value: any) {
...
function removeAttribute(node: any, name: any) {
  if (node.namespaceURI === NS_SVG) {
    name = name.toLowerCase();
    name = svgAdjustAttributes.get(name) || name;
    if (svgForeignAttributes.has(name)) {
...
function setStyles(style: any, values: any) {
